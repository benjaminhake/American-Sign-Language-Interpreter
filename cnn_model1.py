...
labels = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T', 'U', 'V', 'W',
          'X', 'Y']
num_of_image = 600  # we have 100 images for each label
...
# Layer 1
model.add(
    Conv2D(filters=32, kernel_size=(11, 11), padding='Same', activation='relu', input_shape=(200, 200, 1), use_bias=True, strides=(2, 2)))
model.add(BatchNormalization())
model.add(MaxPool2D(pool_size=(2, 2)))  # downsampling
model.add(Dropout(0.25))  # Dropout reduces overfitting
# Layer 2
model.add(Conv2D(filters=48, kernel_size=(7, 7), padding='Same', activation='relu', use_bias=True, strides=(2, 2)))
model.add(BatchNormalization())
...
with open('cnn_model_1000im_epoch30_batch60.json', 'w') as file:
    file.write(json_mod)
